...
import { getDepartmentType, updateStatusDepartmentType, deleteDepartmentType } from '../../../../service/DepartmentType.Service';
...
function MainDepartmentType() {
  ...
  async function fetchData(pageSize, currentPage, search, status) {
    let res = await getDepartmentType(pageSize, currentPage, search, status);
    if (res) {
  ...
  function updateStatus(id, data) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let res = await updateStatusDepartmentType(id, data);
        if (res) {
          if (res.statusCode === 200 && res.taskStatus) {
            Swal.fire({
              icon: 'success',
  ...
  function deleteData(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let res = await deleteDepartmentType(id);
        if (res) {
          if (res.statusCode === 200 && res.taskStatus) {
            Swal.fire({
              icon: 'success',
